...
def parse_to_cable_data(content_bytes_or_str: bytes | str) -> CableData:
    ...
    content = (
        content_bytes_or_str.decode()
        if isinstance(content_bytes_or_str, bytes)
        else content_bytes_or_str
    )

    logger.info(
        f"Parsing {len(content)} bytes of cable info",
    )
    # Parse Event Log
    ...
    downstream_section = content.split("Downstream Bonded Channels")[1].split(
        "Upstream Bonded Channels"
    )[0]
    upstream_section = content.split("Upstream Bonded Channels")[1].split(
        "Downstream OFDM Channels"
    )[0]
    ofdma_downstream_section = content.split("Downstream OFDM Channels")[1].split(
        "Upstream OFDMA Channels"
    )[0]
    ofdma_upstream_section = content.split("Upstream OFDMA Channels")[1].split(
        "Event Log"
    )[0]
...
def append_cable_data_to_db(cable_data: CableData):
    ...
    with get_engine_sync().begin() as connection:
        for table in tables:
            ...
            df.rename(
                columns={
                    ...
                    "ActiveSubcarrier1": "active_subcarrier_1",
                    "ActiveSubcarrier2": "active_subcarrier_2",
                },
                inplace=True,
            )
            df["timestamp"] = int(datetime.now().timestamp())
            write_df_to_db(connection, cable_data[table], table)
